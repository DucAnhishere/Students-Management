...
void inputGrade(Student &student, Lecturer &lecturer)
{
    ...
    if (find(lecturer.lecturer_courses.begin(), lecturer.lecturer_courses.end(), courseToGrade) == lecturer.lecturer_courses.end())
    {
...
{
    // Check if the lecturer is teaching the course
    if (find(lecturer.lecturer_courses.begin(), lecturer.lecturer_courses.end(), course) == lecturer.lecturer_courses.end())
    {
        cout << "You are not teaching this course!" << endl;
        return;
    }
    ...
    for (const Student &student : studentlist)
    {
        ...
        for (const CourseGrade &courseGrade : student.student_courses)
        {
            // If the student is enrolled in the course, print their details
            if (courseGrade.course == course)
            {
                cout << "Student Name: " << student.name << ", ID: " << student.id << ", Grade in " << course << ": " << courseGrade.grade << endl;
                break;
            }
        }
    }
...
void updateGrade(vector<Student> &studentlist, Lecturer &lecturer)
{
    ...
    for (Student &student : studentlist)
    {
        if (student.id == id)
        {
            ...
            for (CourseGrade &courseGrade : student.student_courses)
            {
                if (courseGrade.course == course)
                {
                    cout << "Enter new grade for student " << student.name << " (ID: " << student.id << ") in course " << course << ": ";
                    inputNumber(courseGrade.grade);
                    return;
                }
            }
...
{
    // First, sort the students based on the grade in the given course
    sort(studentList.begin(), studentList.end(), [&courseName](const Student &a, const Student &b)
         {
        // Find the course in each student's list of courses
        auto itA = find_if(a.student_courses.begin(), a.student_courses.end(), [&courseName](const CourseGrade& course) {
            return course.course == courseName;
        });
        auto itB = find_if(b.student_courses.begin(), b.student_courses.end(), [&courseName](const CourseGrade& course) {
            return course.course == courseName;
        });

        // If the course was not found for one of the students, sort that student last
        if (itA == a.student_courses.end()) return false;
        if (itB == b.student_courses.end()) return true;

        // Otherwise, sort by grade in descending order
        return itA->grade > itB->grade; });

    // Then, print the sorted list of students
    for (const Student &student : studentList)
    {
        auto it = find_if(student.student_courses.begin(), student.student_courses.end(), [&courseName](const CourseGrade &course)
                          { return course.course == courseName; });

        // Only print students who are in the course
        if (it != student.student_courses.end())
        {
            cout << "Student: " << student.name << ", Grade in " << courseName << ": " << it->grade << endl;
        }
    }
}
            ...
            while (true)
            {
                if (loginSuccess)
                {
                    ...
                    case 1:
                    {
                        ...
                        {
                            if (lecturer.lecturer_id == username)
                            {
