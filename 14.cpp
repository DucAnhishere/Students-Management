...
public:
    // Constructor
    Person(const string &name, const string &id, const string &phone_number, const string &dob, const string &username, const string &password) : name(name), id(id), phone_number(phone_number), dob(dob), username(username), password(password) {}

    // Getters
    string getName() const { return name; }
    string getId() const { return id; }
    string getSdt() const { return phone_number; }
    ...
    void setSdt(const string &newSdt) { phone_number = newSdt; }
...
public:
    ...
    void setSchool(const string &newSchool) { school = newSchool; }
...
public:
    // Constructor
    Parents(const string &name, const string &parents_id, const string &username, const string &password) : name(name), parents_id(parents_id), username(username), password(password) {}

    // Getters
    string getName() const { return name; }
    string getId() const { return parents_id; }
    string getUsername() const { return username; }
    string getPassword() const { return password; }

    // Setters
    void setName(const string &newName) { name = newName; }
    void setId(const string &newId) { parents_id = newId; }
    void setUsername(const string &newUsername) { username = newUsername; }
    void setPassword(const string &newPassword) { password = newPassword; }
};
...
void show(const Student &s)
{
    ...
    cout << "Class: " << s.getClassName() << endl;
...
void readfile(vector<Student> &studentlist)
{
    ...
    if (save.is_open())
    {
        ...
        while (getline(save, line))
        {
            ...
            // Check if the student has all necessary information
            if (!s.getName().empty() && !s.getId().empty() && !s.getSdt().empty() && !s.getDob().empty() && !s.getSchool().empty() && !s.getClassName().empty())
            {
...
void writeToFile(const vector<Student> &studentlist)
{
    ...
    if (output.is_open())
    {
        for (const Student &s : studentlist)
        {
            ...
            output << s.getId() << endl;
            output << s.getSchool() << endl;
            output << s.getClassName() << endl;
...
void readFileForLecturer(vector<Lecturer> &lecturerlist)
{
    ...
    if (save.is_open())
    {
        ...
        while (getline(save, line))
        {
            ...
            if (!l.getName().empty() || !l.getId().empty() || !l.getSdt().empty() || !l.getDob().empty() || !l.getCourses().empty() || l.getSchool().empty())
            {
